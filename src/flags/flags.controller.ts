...
export class FlagsController {
  ...
  @ApiResponse({
    status: 201,
    description:
      'Flag created with 3 environment configs (all disabled by default)',
  })
  @ApiResponse({ status: 400, description: 'Validation error' })
  @ApiResponse({ status: 401, description: 'Missing or invalid API key' })
  @ApiResponse({
    status: 403,
    description: 'API key does not belong to this tenant',
  })
  @ApiResponse({
    status: 409,
    description: 'Flag key already exists for this tenant',
  })
  create(
  ...
  @ApiOperation({
    summary: 'List flags',
    description:
      'Returns all flags. Add ?environment to include per-env fields (enabled, rolloutPercentage, defaultValue). Add ?status=active|archived to filter.',
  })
  @ApiParam({ name: 'tenantId', description: 'Tenant UUID' })
  @ApiQuery({
    name: 'environment',
  ...
  @Put(':key')
  @ApiOperation({
    summary: 'Configure flag for an environment',
    description:
      'Sets enabled, rolloutPercentage, and targeting rules for a specific environment.',
  })
  @ApiParam({ name: 'tenantId', description: 'Tenant UUID' })
  @ApiParam({ name: 'key', description: 'Flag key' })
